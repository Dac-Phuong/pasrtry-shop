...
<div class="card">
    <div class="d-flex justify-content-between align-items-center">
        ...
        <button type="button" data-bs-toggle="modal" data-bs-target="#kt_modal_update"
            ...
            <tbody>
                @if (isset($list_posts))
                    @foreach ($list_posts as $key => $posts)
                        <tr class="odd">
                            <td>{{ ++$key }}</td>
                            <td><img src="{{ asset('storage/image_posts/' . $posts->image) }}" alt=""
                                    width="100px" height="50px" style="object-fit: contain"></td>
                            <td>{{ $posts->title }}</td>
                            <td>{{ $posts->author }}</td>
                            ...
                            <td>
                                <div class="dropdown" wire:ignore>
                                    ...
                                    <div class="dropdown-menu">
                                        <button data-kt-action="update" data-id={{ $posts->id }}
                                            data-bs-toggle="modal" data-bs-target="#kt_modal_update"
                                            class="dropdown-item" href="javascript:void(0);"><i
                                                class="ti ti-pencil me-1"></i>
                                            Sửa</button>
                                        <button wire:click="delete({{ $posts->id }})" class="dropdown-item"
